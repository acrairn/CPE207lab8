...
export default class App extends Component {
  ...
  loadJsonData = () => {
    ...
    //     this.setState( { transactions: data } );
    //   });
    firebase.firestore().collection('data').onSnapshot(item=>{
      const transaction = []
      item.forEach( res =>{
  ...
  componentDidMount() {
    ...
    firebase.storage().ref('69916571_2457682117663507_178914434742222848_o.jpg').getDownloadURL().then(res => {
  ...
  validateForm = (name,amount) => {
    if (!name || !amount) {
      ...
      window.alert('Please fill only Integer in Ampunt field.')
      return false;
    } else if (+amount === 0) {
